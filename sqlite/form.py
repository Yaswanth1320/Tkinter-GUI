...
def fill_data():
    ...
    if accept == "Accepted":
        ...
        create_query = '''CREATE TABLE IF NOT EXISTS Student_Data
                        (firstname TEXT, lastname TEXT, title TEXT,
                        age INT, nationality TEXT, registration_status TEXT,
                        num_of_courses INT, num_of_semester INT)
                    '''
        conn.execute(create_query)

        insert_query ='''INSERT INTO Student_Data (
                        firstname, lastname, title, age, nationality, 
                        registration_status, num_of_courses, num_of_semester) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)'''
        data_tuple = (firstname, lastname, title, Nationality, age, resgistration, numcourses, numsemesters)
        cursor = conn.cursor()
